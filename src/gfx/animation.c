...
AnimationSequence animation_load_sequence(const char *base_name, char direction,
                                          float fps, int loop) {
  ...
  for (int frame = 1; frame <= 4; frame++) {
    ...
    if (direction == 'L' || direction == 'R') {
      snprintf(filename, sizeof(filename), "assets/img/%s%d%c.png", base_name,
               frame, direction);
    } else {
      snprintf(filename, sizeof(filename), "assets/img/%s%d.png", base_name,
               frame);
    }
...
DirectionalAnimationSet animation_load_directional(const char *base_name,
                                                   float fps, int loop) {
...
void directional_animation_update(DirectionalAnimationSet *set,
                                  float delta_time) {
  animation_sequence_update(&set->left, delta_time);
  animation_sequence_update(&set->right, delta_time);
}

void animation_sequence_render(AnimationSequence *seq, float x, float y,
  ...
  Rectangle dest = {x - (frame->width * scale) / 2, y - frame->height * scale,
                    frame->width * scale, frame->height * scale};

  DrawTexturePro(frame->texture, source, dest, (Vector2){0, 0}, 0, tint);
}

void directional_animation_render(DirectionalAnimationSet *set, float x,
                                  float y, float scale, Color tint) {
