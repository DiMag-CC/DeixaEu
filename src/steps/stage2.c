...
static Texture2D texturaPersonagemReserva;
...
void initStage2(Stage2 *stage) {
    ...
    texturaPersonagemReserva = LoadTexture("assets/img/characterMovingR1.png");

    if (stage->bgSand.id == 0 || texturaCaranguejo.id == 0 || texturaBuraco.id == 0 || texturaPersonagemReserva.id == 0) {
...
void unloadStage2(Stage2 *stage) {
    ...
    UnloadTexture(texturaPersonagemReserva);
...
static void updateSand(Stage2 *stage, Player *player, float deltaTime) {
    if (player->lives <= 0) {
        if (IsKeyPressed(KEY_ENTER)) {
            ...
            freeStage2Queue(&stage->obstacleQueue);
            initStage2Queue(&stage->obstacleQueue);
            printf("[STAGE 2] Reset interno executado com sucesso!\n");
        }
        return; 
    }
    ...
    // Hitbox calibrada para o corpo do jogador
...
void drawStage2(Stage2 *stage, Player *player) {
    ...
    if (stage->mode == STAGE2_MODE_SAND) {
        if (texturaPersonagemReserva.id > 0) {
            Rectangle source = { 0.0f, 0.0f, (float)texturaPersonagemReserva.width, (float)texturaPersonagemReserva.height };
            Rectangle dest = {
                player->position.x,
                player->position.y, 
                player->width,
            ...
            Vector2 origin = { 0.0f, 0.0f };
            DrawTexturePro(texturaPersonagemReserva, source, dest, origin, 0.0f, WHITE);
        } else {
